...
class CSVTimeSeriesFile(CSVFile):
    ...
    def get_data(self):       #!controlla e scarta già le righe vuote e non valide!
        ...
        if self.can_read:
            ...
            lista_annidata = []
            for value in data:
                #lista.annidata.append solo se tale valore è numerico, non vuoto
            ...
            for ts in range(len(timestamps) - 1):
                if timestamps[ts + 1] < timestamps[ts]:
                    raise ExamException("I timestamps forniti non sono in ordine...")
...
def compute_daily_max_difference(list_of_lists):
    ...
    def add_esc(giorno):             
        if len(giorno) == 1 or len(giorno) == 0:
            escursioni.append(None)             #l'escursione in caso di singola misurazione è None
        else:
    ...
    for i in range(len(list_of_lists)):
        ...
        if list_of_lists[i] == list_of_lists[-1]:
            if (int(list_of_lists[i][0]) - (int(list_of_lists[i][0]) % 86400)) == (int(list_of_lists[i-1][0]) - (int(list_of_lists[i-1][0]) % 86400)):
